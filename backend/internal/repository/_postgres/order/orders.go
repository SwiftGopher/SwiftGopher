...
func (r *OrderRepository) Create(ctx context.Context, o *modules.Order) error {
	query := `
		INSERT INTO orders (
			id, client_id, courier_id, pickup_lat, pickup_lng, pickup_address, delivery_address,
...
func (r *OrderRepository) GetByID(ctx context.Context, id string) (*modules.Order, error) {
	query := `
		SELECT id, client_id, COALESCE(courier_id::text, '') as courier_id, pickup_lat, pickup_lng, pickup_address, delivery_address,
		       status, price, created_at, updated_at
		FROM orders
		WHERE id = $1`
...
func (r *OrderRepository) GetHistory(ctx context.Context, orderID string) ([]*modules.OrderHistory, error) {
	...
	if history == nil {
		return []*modules.OrderHistory{}, nil
	}
...
func (r *OrderRepository) GetMyOrders(ctx context.Context, userID string) ([]*modules.Order, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("orderRepository.GetMyOrders: %w", err)
	}
	defer rows.Close()
...
func (r *OrderRepository) GetByIDs(ctx context.Context, ids []string) ([]*modules.Order, error) {
	if len(ids) == 0 {
		return []*modules.Order{}, nil
	}
	...
	rows, err := r.db.DB.Query(ctx, query, ids)
	if err != nil {
		return nil, fmt.Errorf("orderRepository.GetByIDs: %w", err)
	}
	defer rows.Close()
...
func (r *OrderRepository) GetByCourierID(ctx context.Context, courierID string) ([]*modules.Order, error) {
	query := `
		SELECT id, client_id, COALESCE(courier_id::text, '') as courier_id, pickup_lat, pickup_lng, pickup_address, delivery_address,
		       status, price, created_at, updated_at
		FROM orders
	...
	if err != nil {
		return nil, fmt.Errorf("orderRepository.GetByCourierID: %w", err)
	}
	defer rows.Close()
